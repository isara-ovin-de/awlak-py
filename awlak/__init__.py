...
class Awlak:
    ...
    def _shutdown_loop(self) -> None:
        ...
        for future in list(self._pending_api_calls):
            if not future.done():
                try:
                    ...
                    future.result(
                        timeout=self.api_timeout + 1
                    )  # Add a bit more timeout than API
                except asyncio.TimeoutError:
                    self.logger.warning(
                        f"A task did not complete in time during shutdown: {future}"
                    )
                except Exception as e:
                    self.logger.error(
                        f"Exception while waiting for task during shutdown: {e}"
                    )
        ...
        if not self._loop.is_closed():
            ...
            try:
                ...
                if remaining_tasks:
                    self.logger.info(
                        f"Cancelling {len(remaining_tasks)} remaining tasks before closing loop."
                    )
    ...
    def _get_code_context(
        self, frame, lines_before: int = 3, lines_after: int = 3
    ) -> List[str]:
    ...
    def _get_local_variables(self, frame) -> Dict[str, str]:
        ...
        try:
            ...
            local_var_names = code_obj.co_varnames[
    ...
    async def _send_to_api(self, data: Dict[str, Any]) -> bool:
        ...
        self.logger.info(
            f"Starting API call process for {event_type} '{event_title}'..."
        )
        ...
        async def attempt_request(session: ClientSession, retries: int) -> bool:
            ...
            self.logger.error(
                f"API call failed for {event_type} '{event_title}' after {retries + 1} attempts."
            )
            return False
    ...
    def capture_exception(
        self, exception: Exception, severity: str = ERROR, **kwargs
    ) -> None:
        ...
        if severity not in self._VALID_SEVERITIES:
            raise ValueError(
                f"Invalid severity. Must be one of {self._VALID_SEVERITIES}"
            )
        ...
        data = {
            ...
            "kwargs": {
                ...
                if k not in ("title", "tags", "severity")
            },
        }
        ...
        if not self.api_key:
            ...
            if self._own_loop:
                future = asyncio.run_coroutine_threadsafe(
                    self._send_to_api(data), self._loop
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        capture_exception(
            e, title="Division Error", severity=ERROR, tags=["math"], context="Testing"
        )

    # Example with event capture
    capture_event(
        "User logged in", title="User Event", severity=INFO, tags=["auth"], user_id=123
    )
